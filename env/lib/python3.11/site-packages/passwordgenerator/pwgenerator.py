    ...
    for i in range(number_of_elements):
        ...
        if i == int_position:
            ...
            password += get_random_word(
                dictionary, min_word_length, max_word_length
            ).title()
...
def main():
    ...
    parser.add_argument(
        ...
        "--min_word_length",
        type=int,
        help="Minimum length for each word",
        default=3,
    )
    parser.add_argument(
        "-x",
        "--max_word_length",
        type=int,
        help="Maximum length for each word",
        default=8,
    )
    parser.add_argument(
        "-i",
        "--max_int_value",
        type=int,
        help="Maximum value for the integer",
        default=1000,
    )
    parser.add_argument(
        "-e",
        "--number_of_elements",
        type=int,
        help="Number of elements in the password (ie. 4 = 3 words + 1 integer)",
        default=4,
    )
    parser.add_argument(
        "-s",
